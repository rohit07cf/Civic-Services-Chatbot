...
function EditIntentController($rootScope, $scope, Agent, AgentEntities, Intent, Expressions, Expression, Parameter, Parameters, Entities, UniqueIntentEntities,Responses, Response ) {
  ...
  $scope.updateParameterEntity = function(param_id, entity_id,paramValue) {
    Parameter.update({parameter_id: param_id}, {parameter_id: param_id, entity_id: entity_id,parameter_value :paramValue}).$promise.then(function() {
      loadUniqueIntentEntities();
    });
  }
