...
function TrainingController($scope, $rootScope, $interval, $http, Rasa_Status, Agent, Intents, Expressions, Regex, ExpressionParameters, Rasa_Config, EntitySynonymVariantsByEntity, AllSynonymVariants, IntentExpressions,yaml,AgentEntities,AgentActions) {
  ...
  $scope.trainRasaCore = function(){
    let data = new Blob([$scope.domain_yml], {type: 'text/plain'});

    let stories_data = new Blob([$scope.stories_md], {type: 'text/plain'});

    var formData = new FormData();
    formData.append("stories", stories_data , "stories.md");
    formData.append("domain" , data , "domain_freeText.yml")

     var request = new XMLHttpRequest();
    request.onload = function(){
      responseText = request.responseText
      responseJoin = JSON.parse(responseText);
      console.log(responseText)
    };

    request.open("POST", "http://localhost:5005/webhooks/crest/train");
    ...
    AgentEntities.query({agent_id: agent_id},function(allEntities) {
        ...
        if(requiredSlots.length>0){
          ...
          var slots_yml_str = requiredSlots.map(function(slot) {
            let entity_values_str = '';
            console.log(slot)
            if(slot['entity_values']!= null && slot['entity_values']!= undefined  && slot['entity_values']!=''){
              const entity_values = slot['entity_values'].split(',');
              console.log("entity_values")
              console.log(entity_values)
              entity_values_str = ',\"values\":'+JSON.stringify(entity_values);
            }else{
              entity_values_str = '';
            }
            //console.log( "\""+slot['entity_name']+"\":{\"type\":\""+slot['slot_data_type']+"\""+entity_values_str+"}");
            return "\""+slot['entity_name']+"\":{\"type\":\""+slot['slot_data_type']+"\""+entity_values_str+"}";
          }).join(",");
  ...
  function generateData(regex, intents, expressions, params, synonyms) {
    ...
    for (var synonym_i = 0; synonym_i < synonyms.length; synonym_i++) {
      if(!!synonyms[synonym_i].synonyms)
