...
function ActionsController($rootScope, $scope, $http, Actions, Response, ActionResponses, Agent, AgentActions) {
  ...
  Actions.get({ action_id: $scope.$routeParams.action_id }, function (data) {
    ...
    else if ($scope.action.action_name.startsWith('utter_')) {
    ...
    else if ($scope.action.action_name.startsWith('action_')) {
      $scope.action.action_name_prefix = 'action_';
      $scope.action.action_name = $scope.action.action_name.substring('action_'.length, $scope.action.action_name.length);
    }
    else {
      $scope.action.action_name_prefix = 'utter_';
      $scope.action.action_name = $scope.action.action_name.substring('utter_'.length, $scope.action.action_name.length);
  ...
  $scope.addButton = function (action) {
    ...
    const btnString = action.button_text.split(":")
    const button = { "seq": seq }
    button['title'] = ''
    button['payload'] = ''
    if (!!btnString[0] && !btnString[1] && btnString.length <= 2) {
      button['type'] = btnString[0]
    } else if (!!btnString[0]) {
      button['title'] = btnString[0]

      if (!!btnString[1]) {
        button['payload'] = btnString[1]
      }
      if (!!btnString[2]) {
        button['action'] = btnString[2]
      }
    }
    $scope.buttonsArray.push(button);
