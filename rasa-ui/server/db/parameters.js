...
function updateParameter(req, res, next) {
  ...
  db.none('update parameters set entity_id=$1 , parameter_value=$3 where parameter_id=$2',
    [req.body.entity_id, parseInt(req.params.parameter_id),req.body.parameter_value])
    .then(function () {
      res.status(200)
        .json({
          status: 'success',
