...
function createEntity(req, res, next) {
  ...
  db.none('insert into entities(entity_name, agent_id, slot_data_type , entity_values) values(${entity_name},${agent_id},${slot_data_type},${values})',
    req.body)
    .then(function () {
      res.status(200)
        .json({
...
function updateEntity(req, res, next) {
  db.none('update entities set entity_name=$1, agent_id=$3, slot_data_type=$4 , entity_values = $5 where entity_id=$2',
    [req.body.entity_name, parseInt(req.params.entity_id),parseInt(req.body.agent.agent_id), req.body.slot_data_type , req.body.entity_values])
    .then(function () {
      res.status(200)
        .json({
          status: 'success',
