...
//var await = require('asyncawait/await');
 ...
 function parseRasaRestRequest(req, res, next, agentObj) {
  ...
  messageObj.user_id = req.body.sender;//req.jwt.username;
  messageObj.user_name = req.body.sender;//req.jwt.name;
  messageObj.message_text = req.body.message;
  ...
  if(agentObj == undefined ){
    ...
    ' basic_auth_password, rasa_core_enabled from agents where agent_name = $1', agent_name).then(function (data) {
      ...
      parseRestRequest(req, res, next, data[0]);
    }).catch(function(err){
    ...
    parseRestRequest(req, res, next, agentObj);
  }
}

function parseRestRequest(req, res, next, agentObj) {
  var core_url = global.rasacoreendpoint +"/webhooks/rest/webhook";
  ...
  var responseBody = await (rasaCoreRestRequest(req,"parse",JSON.stringify(req.body)));
}
  ...
  function rasaCoreRestRequest(req,RasaUri,reqBody){
    return new Promise((resolve, reject) => {
        request({
          method: "POST",
          uri: RasaUri,
  ...
  */
  ...
  async function fetchActionDetailsFromWebhook(req,rasa_core_response, agentObj){
    return new Promise((resolve, reject) => {
      request.post({
...
*/
  ...
  module.exports = {
    ...
    parseRasaRestRequest:parseRasaRestRequest,
